...
const mapPredicition = (predicition: string): "neutral" | "conservative" | "liberal" => {
  switch (predicition) {
    case "right":
      return "conservative"
    case "left":
...
(async () => {
  ...
  if (pageResults.state == "success" && pageResults.article) {
    ...
    addReadArticle({url: cleanUrl(document.location.href), name: pageResults.article.title }, rPrediction, auth.user, firestore);
  }
})();
