...
const foxNewsExtractor = (pageContent: Document): Extraction => {
  const featVideo = pageContent.getElementsByClassName("featured featured-video")[0];
  ...
  const articleText = genericCleaner(article.innerText.replaceAll(featVideo.innerText, "")); // remove the featured video content from string
  // @ts-ignore not sure why
