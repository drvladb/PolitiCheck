...
const addOneLifetimeStats = async (bias: "neutral" | "conservative" | "liberal", user: User, db: Firestore) => {
    ...
    const newData = { ...data, stats: {...data.stats, lifetime: {...data.stats.lifetime, [bias]: data.stats.lifetime[bias] + 1}}}
    // console.log(newData)
    await setUserData(user, db, newData);
}

const addReadArticle = async (article: { url: string, name: string }, bias: "neutral" | "conservative" | "liberal", user: User, db: Firestore) => {
    await addArticle(user, db, article.url, article.name, bias);
    await addOneLifetimeStats(bias, user, db);
}

export {checkUserData, addReadArticle, getArticles, getUserData, addOneLifetimeStats}